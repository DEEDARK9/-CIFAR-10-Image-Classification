...
📓 Explore the Notebook  ·  🚀 Live Demo  ·  📊 Key Results
...
How much does a pretrained backbone actually help compared to training from scratch when both models share the same training setup?
...
<img width="500" alt="Live Demo — upload interface with model selector" src="assets/screenshot_live_demo.png"/>

<img width="500" alt="Model Comparison — benchmark table and key findings" src="assets/screenshot_model_comparison.png"/>
...
torchvision (MobileNetV2, ResNet-18) + Custom CNN

Notebook-only Models

EfficientNet-B0, ViT (Small, custom implementation) — explored in notebook, not deployed
...
Matplotlib (deployed) · Seaborn (notebook)

Interpretability

Grad-CAM with PyTorch hooks — CLI (gradcam.py) + notebook only

Demo App

Gradio ≥5.29 on Hugging Face Spaces

Model Weights

Hugging Face Hub (Deepak/cifar10-models)

Hardware

Auto-detected: CUDA / Apple Silicon MPS / CPU
...
Custom CNN, MobileNetV2, ResNet-18 training & inference

✅ Deployed (live demo + CLI)

EfficientNet-B0, ViT-Small

📓 Notebook-only (not deployed)

RandomCrop, HFlip, CutOut, MixUp, CutMix augmentation

📓 Notebook-only training pipeline
...
🛠️ CLI + notebook (gradcam.py) — not in the Gradio app UI

INT8 dynamic quantisation

🧪 Experiment (notebook, Section 13)

CLI single-image / batch / full test-set inference

✅ Deployed (predict.py)

Hugging Face Hub runtime weight loading
...
Similar vehicle structure at 32×32
...
Fine-grained mammal similarity
...
Challenging low-resolution silhouettes
...
Seed setup, device detection, hyperparameter config

2

Data Preparation & Augmentation

Dataset loading, RandomCrop/HFlip/CutOut, MixUp & CutMix

3

Model Architectures

Custom CNN, MobileNetV2, ResNet-18, EfficientNet-B0, ViT (Small)

4

Training Pipeline

Unified loop with cosine annealing and AMP support

5

Train All Models

Controlled comparisons across all five architectures

6

Progressive Unfreezing

MobileNetV2 backbone fine-tuning schedule

7

Test Set Evaluation

Full test-set accuracy and class-level metrics
...
Loss, accuracy, and LR schedule visualisation
...
Efficiency & Deployment Analysis

Parameters, size, latency, and throughput

12

Model Quantization (INT8)

Dynamic quantisation experiments

13

Save Experiment Artifacts

Export config, results, and metadata

14

Final Summary & Conclusions

Consolidated findings across all models
...
Lead — ML pipeline, deployment, Gradio app
...
data/ (CIFAR-10 raw files) and checkpoints/ (local .pth files) are git-ignored — the dataset auto-downloads via torchvision on first run, and deployed model weights are hosted on Hugging Face Hub and downloaded automatically at runtime. No binaries are committed to this repo.
...
Benchmark metrics (single source of truth)

benchmark_data.py

Training metadata & retrain history
...
The custom domain (cifar10.Deepak.dev) is a DNS alias for the Hugging Face Space and depends on the Space container staying awake; if the Space is asleep or down, the custom domain will be unavailable too.
...
PyTorch — Deep learning framework and pretrained model weights.

Hugging Face — Model hosting and Spaces deployment infrastructure.
